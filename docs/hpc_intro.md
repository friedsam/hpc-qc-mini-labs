...
MPI (Message Passing Interface) is a standard for writing programs that run as multiple independent processes, called ranks. This enables parallel computing, allowing programs to run across multiple processors or computers by explicitly passing data as messages between ranks. 
...
OpenMP (Open Multi-Processing) is a threading model for parallelism within a single process. It simplifies parallel programming by allowing developers to easily parallelize loops and code sections. This makes programs run across multiple CPU cores or even GPUs for complex tasks and works on desktops, servers and supercomputers.
...
When tasks are too small, overhead can dominate.
...
These serial sections limit maximum achievable speedup.
